...
type WhyProps = { title?: string; highlight?: string; bodyHtml?: string; imageUrl?: string; cards?: WhyCard[] };

const Why = ({
  title = 'Built for packaging.',
  highlight = 'Powered by AI.',
  bodyHtml,
  imageUrl = '/home/QC_Assist_Screen_Grafik.webp',
  cards,
}: WhyProps) => {
  const card = cards?.[0];
    ...
    <section className="text-foreground w-full px-2 py-10 md:py-16">
      <div className="grid w-full grid-cols-1 gap-8 lg:grid-cols-[1fr_auto] lg:items-start">
        <div>
          ...
          <div className="mt-10 max-w-[860px] text-xl leading-7 font-semibold tracking-normal text-[#262626]">
            {bodyHtml ? (
          ...
          <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[0.95fr_1fr] lg:items-stretch">
            <div className="bg-background flex min-h-[330px] flex-col rounded-lg p-8 shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.102),0px_10px_15px_-3px_rgba(0,0,0,0.102)]">
              ...
              <div className="mt-6 max-w-[540px] text-xl leading-7 font-semibold tracking-normal text-[#303030]">
                {card?.bodyHtml ? (
                  <HtmlContent html={card.bodyHtml} />
                ) : (
                  <>
